...
import {
  ...
  Smile, Sparkles, Cloud, Check, TreePine, Calendar, BarChart3, Award, Plus,
  ShieldCheck, RefreshCw, LogIn, LogOut, User
} from 'lucide-react';
...
const embedDatabase: Record<string, Record<string, string[]>> = {
  ...
  "小小吃": {
    "中式/台式": ["便當", "水餃/鍋貼", "牛肉麵", "滷肉飯", "鹹酥雞", "串燒", "別偷懶給我自己煮^_^", "i珍食/友善食光", "永和豆漿", "百元小火鍋", "鍋燒意麵", "雞肉飯", "夜市", "麻辣鴨血臭豆腐", "炸的那種超豆腐", "麵線", "羹麵/羹飯", "藥燉排骨", "生煎包", "擔仔麵", "肉圓", "碗粿", "蔥抓餅", "大腸包小腸", "潤餅", "炒麵", "蒸餃", "筒仔米糕", "羊肉炒麵", "滷味", "鹽水雞"],
...
interface GrowthForestSectionProps {
...
// 成長森林子元件
function GrowthForestSection({ onOpenAuthModal, user, handleLogout }: GrowthForestSectionProps) {
    ...
    try {
      ...
      if (user) {
        ...
        if (records) {
          ...
          const todayFound = records.find(r => r.record_date === todayStr);
          if (todayFound) {
            setTodayRecord({
              water_goal: todayFound.water_goal ?? false,
              mood: todayFound.mood ?? 8,
              read_book: todayFound.read_book ?? false,
              studied: todayFound.studied ?? false,
              exercised: todayFound.exercised ?? false,
              coded: todayFound.coded ?? false,
              job_search: todayFound.job_search ?? false,
              custom_1: todayFound.custom_1 ?? false,
              custom_1_name: todayFound.custom_1_name ?? '輸入自訂項目',
              custom_2: todayFound.custom_2 ?? false,
              custom_2_name: todayFound.custom_2_name ?? '輸入自訂項目',
              custom_3: todayFound.custom_3 ?? false,
              custom_3_name: todayFound.custom_3_name ?? '輸入自訂項目',
              custom_4: todayFound.custom_4 ?? false,
              custom_4_name: todayFound.custom_4_name ?? '輸入自訂項目',
              note: todayFound.note ?? ''
            });
          }
        }
      } else {
        setUserEmail(null);
        // 未登入時回退至 localStorage 讀取
  ...
  async function updateRecord(updatedFields: Partial<typeof todayRecord>) {
                ...
                {allRecords.map((record) => (
                  <div key={record.id || record.record_date} className="bg-white/90 dark:bg-slate-800/90 backdrop-blur p-5 rounded-2xl border border-emerald-100 dark:border-slate-700 shadow-sm space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs">
                      {record.studied && <span className="bg-emerald-50 dark:bg-emerald-950/50 text-emerald-700 dark:text-emerald-300 px-2.5 py-1 rounded-lg font-medium">學習</span>}
                      {record.job_search && <span className="bg-indigo-50 dark:bg-indigo-950/50 text-indigo-700 dark:text-indigo-300 px-2.5 py-1 rounded-lg font-medium">求職</span>}
                      {record.coded && <span className="bg-teal-50 dark:bg-teal-950/50 text-teal-700 dark:text-teal-300 px-2.5 py-1 rounded-lg font-medium">寫作業/工作</span>}
                      {record.read_book && <span className="bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300 px-2.5 py-1 rounded-lg font-medium">閱讀</span>}
                      {record.exercised && <span className="bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-300 px-2.5 py-1 rounded-lg font-medium">運動</span>}
                      {record.water_goal && <span className="bg-cyan-50 dark:bg-cyan-950/50 text-cyan-700 dark:text-cyan-300 px-2.5 py-1 rounded-lg font-medium">喝水</span>}
...
export default function Home() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    else setAuthMsg('登入連結已發送給您，請至信箱收驗證信！');
  };
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-slate-950 text-stone-900 dark:text-slate-100 transition-colors">
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl max-w-sm w-full border border-stone-200 dark:border-slate-800 shadow-2xl space-y-4">
            <h3 className="text-lg font-bold text-stone-800 dark:text-slate-100">登入 Supabase 儲存進度</h3>
            <p className="text-xs text-stone-500 dark:text-slate-400">
              輸入 Email 即可接收無密碼登入連結 (Magic Link)，讓你在不同裝置間輕鬆同步打卡進度。
            </p>
            <form onSubmit={handleLogin} className="space-y-3">
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-stone-200 dark:border-slate-700 bg-stone-50 dark:bg-slate-800 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              <button
                type="submit"
                className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-sm transition-all"
              >
                寄送登入驗證信
              </button>
            </form>
            {authMsg && <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium text-center">{authMsg}</p>}
            <button
              onClick={() => setShowAuthModal(false)}
              className="w-full py-2 rounded-xl text-xs text-stone-400 hover:text-stone-600 dark:hover:text-slate-200 transition-colors"
            >
              關閉
            </button>
          </div>
